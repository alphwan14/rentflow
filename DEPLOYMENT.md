...
- In the **Africa's Talking dashboard**, set the SMS **Delivery Report callback URL** to:
  ...
  (token in the URL **path**, preferred; `?token=<...>` also authenticates so a
  legacy-shaped URL never silently breaks. AT does not sign callbacks, so the
  endpoint layers three checks: constant-time token comparison, correlation
  against a known in-flight message id, and an optional
  `DELIVERY_REPORT_ALLOWED_IPS` source allowlist once AT support confirms
  their egress ranges.)
